...
//    Mult mult;
    ...
    auto res = multiply(this->toVector(), l);//mult.fft_mul(this->toVector(), l);

    std::list<int> resList;

    bool isFirstZeros = *(rbegin(res)) == 0 ? true : false;
    for(auto it = rbegin(res); it != rend(res); ++it)
    {
        if(isFirstZeros && *it != 0) isFirstZeros = false;
        if(!isFirstZeros) resList.push_front(*it);
    }
